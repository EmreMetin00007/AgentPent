...
Nmap ile açık web veya proxy-benzeri port bulduktan sonraki doğrulama örneği:
```json
{
  "tool_calls": [
    {"tool": "httpx", "params": {"target": ["http://65.61.137.117", "http://65.61.137.117:8080", "http://65.61.137.117:3128"]}}
  ]
}
```
...
3. `nmap quick` ile 80, 443, 8080, 8443 veya 3128 gibi HTTP/proxy-benzeri portlar görünürse `httpx` ile tam URL kullanarak doğrula
4. 3128 veya proxy benzeri cevaplarda bunu uygulama gibi varsayma; önce sadece servis/proxy doğrulaması yap
5. Yalnızca pozitif kanıt varsa ek araç çağrısı yap
6. Aynı araçları aynı parametrelerle tekrar etme
...
- Nmap ile açık gördüğün HTTP/proxy-benzeri portları final bulgularında tek tek belirt
- `httpx` başarısız olsa bile nmap ile doğrulanan açık port bilgisini kaybetme
